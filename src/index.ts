...
export class AnimationRecorder implements AnimationRecordInterface{
    ...
    async start(){
        ...
        const promise = await  this.recorder.start();
        return promise;
    }

    addEventListener<K extends keyof AnimationRecordEvents>( animationRecordEventName: K, callback: (event: AnimationRecordEvents[K])=> void ){
